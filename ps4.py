...
def r_squared(y, estimated):
    ...
    """
    predicted, observed = np.array(estimated), np.array(y)
    error = ((predicted - observed)**2).sum()
    meanError = error/len(observed)
    return float(1 - (meanError/np.var(observed)))
...
def evaluate_models_on_training(x, y, models):
    ...
    for i in range(len(models)):
        estYvals.append(np.polyval(models[i], x))
        r2.append(r_squared(y, estYvals[i]))

    ind = r2.index(max(r2))
    yVals, bestFitDeg =estYvals[ind], len(models[ind])-1
    plt.plot(x, y, "bo")
    plt.plot(x, yVals, "r-", )
    plt.ylabel("Temperature")
    plt.xlabel("Years")
    plt.title(str(bestFitDeg)+" degree model with \n R-square: "+str(r2[ind]))
    plt.show()
